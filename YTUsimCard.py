...
class MyWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.setWindowTitle(_translate("MainWindow", "YTUsimWriter"))
    ...
    def setFPLMN(self):
        ...
        for i in range(len(self.cmds)):
            ...
            fplmns = valid
            tmp = ""
            for n in range(min(len(fplmns), 8)):
                plmn = fplmns[n]
                if (len(plmn) == 5):
                    plmn = plmn + 'F'
                tmp = tmp + plmn[1] + plmn[0] + plmn[5] + plmn[2] + plmn[4] + plmn[3] + ' '
            for n in range(8 - len(fplmns)):
                tmp = tmp + "FFFFFF "
            tmp = tmp[0:55]
            newcmd = cmd[0:11] + tmp + cmd[66:]
            self.cmds[i] = [newcmd, cmdlist[1]]
        return
    ...
    def setKEY(self):
        ...
        for i in range(len(self.cmds)):
            ...
            if len(key) != 32:
                return
            newcmd = cmd[0:15] + key.lower() + cmd[47:]
            self.cmds[i] = [newcmd, cmdlist[1]]
        return
    ...
    def cmdsToText(self):
        ...
        for cmd in self.cmds:
            tmp1 = "{0}<br>".format(cmd[0])
            if re.search(pat, cmd[0]):
    ...
    def loadCommandFile(self):
        ...
        if ok:
            ...
            for item in items:
                ...
                if cmdstr[0] == "":
                    continue
                cmd = re.split("COMMAND-STATUS-WORD", cmdstr[0])
                if len(cmd) < 2:
                    cmd.append("")
    ...
    def readCard(self):
        ...
        # read spn
        spn_cmds = [
            ["A0A4000002 3f00", "9f17"],
            ["A0A4000002 7f20", "9f17"],
            ["A0A4000002 6f46", "9f0f"],
            ["A0B0000011", "9000"],
        ]
        for cmd in spn_cmds:
            instruction = cmd[0].replace(" ", "")
